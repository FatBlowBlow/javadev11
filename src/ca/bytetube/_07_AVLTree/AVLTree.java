...
public class AVLTree<E> extends BinarySearchTree<E> {
    ...
    private static class AVLNode<E> extends Node<E>{
        ...
        public int balanceFactor(){//每一个节点的平衡因子，是针对树当中的每一个节点的，所以封装在AVLNode中
    ...
    public void afterAdd(Node<E> node){//新添加的节点，node is leaf node
        while ((node = node.parent) != null){//一直向上找
            if (isBalanced(node)) {//判断每个节点是否平衡
                //更新高度，更新的是每一个节点的高度，所以封装在AVLNode中
                updateHeight(node);
            }else {
                rebalance2(node);
                break;//从下向上调平，只需调平一次
    ...
    private void rotate(Node<E> r,//需要拿到最开始子树的根节点, 它的parent，作为d点的parent
    ...
    public void afterRemove(Node<E> node){//父节点 or 祖先节点中，只有一个节点可能失衡
        while ((node = node.parent) != null){//一直向上找
            if (isBalanced(node)) {//判断每个节点是否平衡
                ...
                rebalance(node);//去掉break，因为可能祖先节点失衡，所以要一路向上找，找到可能失衡的节点并调平
            }
        }
    }
